...
describe("ConversationViewer", () => {
  describe("render width safety", () => {
    ...
    it("no line exceeds width with tab-indented content", () => {
      const tabby = [
        " *\t\theap_rescan\t\t- restart a relation scan",
        " *\t\theap_endscan\t- end relation scan",
        " *\t\theap_getnext\t- read the next tuple in scan order",
      ].join("\n");
      const messages = [
        { role: "toolResult", toolUseId: "t1", content: [{ type: "text", text: tabby }] },
      ];
      for (const w of widths) {
        const viewer = new ConversationViewer(
          mockTui(30, w), mockSession(messages), mockRecord(), undefined, ansiTheme(), vi.fn(),
  ...
  describe("safety net against upstream wrapTextWithAnsi bugs", () => {
    ...
    it("normalizes tabs before width-sensitive rendering", () => {
      const w = 40;
      const messages = [
        {
          role: "toolResult",
          toolUseId: "t1",
          content: [{ type: "text", text: "*\t\theap_rescan\t\t- restart a relation scan" }],
        },
      ];
      const viewer = new ConversationViewer(
        mockTui(30, w), mockSession(messages), mockRecord(), undefined, ansiTheme(), vi.fn(),
