...
export class ConversationViewer implements Component {
  ...
  private buildContentLines(width: number): string[] {
    ...
    for (const msg of messages) {
      if (msg.role === "user") {
        const text = normalizeRenderableText(typeof msg.content === "string"
          ? msg.content
        ...
        for (const c of msg.content) {
          if (c.type === "text" && c.text) textParts.push(normalizeRenderableText(c.text));
        ...
        const text = normalizeRenderableText(extractText(msg.content));
        ...
        if (needsSeparator) lines.push(th.fg("dim", "───"));
        lines.push(truncateToWidth(th.fg("muted", `  $ ${normalizeRenderableText(bash.command)}`), width));
        if (bash.output?.trim()) {
          const normalizedOutput = normalizeRenderableText(bash.output);
          const out = normalizedOutput.length > 500
            ? normalizedOutput.slice(0, 500) + "... (truncated)"
            : normalizedOutput;
