...
import { type DisposableComponent, wrapComponentSafely } from "./safe-component.js";
...
export type UICtx = {
  ...
  setWidget(
    key: string,
    content: undefined | ((tui: any, theme: Theme) => DisposableComponent),
  ...
  update() {
    ...
    if (!this.widgetRegistered) {
      this.uiCtx.setWidget("agents", (tui, theme) => {
        this.tui = tui;
        return wrapComponentSafely({
          render: (_width: number) => this.renderWidget(tui, theme),
          invalidate: () => {
          ...
          label: "subagents widget",
          onError: (error) => {
            const detail = error instanceof Error ? error.message : String(error);
            this.uiCtx?.setStatus("subagents", "widget error");
            console.error(`[subagents widget] render failed: ${detail}`);
          },
        });
