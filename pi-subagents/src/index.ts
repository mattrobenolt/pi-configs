...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerMessageRenderer<NotificationDetails>(
    ...
    (message, { expanded }, theme) => {
      ...
      const truncate = (s: string) => truncateToWidth(s, w);
      const all = [d, ...(d.others ?? [])];
      const truncated = all.map(renderOne).join("\n").split("\n").map(truncate).join("\n");
...
- Use isolation: "worktree" to run the agent in an isolated git worktree (safe parallel file modifications).`,
  ...
  async function viewAgentConversation(ctx: ExtensionCommandContext, record: AgentRecord) {
    ...
    const { wrapComponentSafely } = await import("./ui/safe-component.js");
    ...
    await ctx.ui.custom<undefined>(
      (tui, theme, _keybindings, done) => {
        return wrapComponentSafely(
          new ConversationViewer(tui, session, record, activity, theme, done),
          {
            label: "subagent viewer",
            onError: (error) => {
              const detail = error instanceof Error ? error.message : String(error);
              ctx.ui.notify(`Subagent viewer render failed: ${detail}`, "warning");
            },
          },
        );
      },
