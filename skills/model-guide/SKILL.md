---
name: model-guide
description: Pick which model a subagent should run on, for pi model routing. Use when deciding which model to assign a task to, choosing a model for a subagent, forming a cross-family review quorum, or evaluating whether a model's benchmark claims are trustworthy. Triggers on "which model", "pick a model", "choose a model", "model for this task", "model routing", "subagent model", "model quorum", "beats GPT-5.5", or any model-selection decision. Encodes Matt's defaults (GLM 5.2 default, open-weight bias, cross-family quorums, Fable as a conscious juggernaut, M3 never for coding) and the neutral-eval skepticism every model in the guide failed.
---
...
| `openai-codex/gpt-5.4-mini` | 40 | execution, not planning | Vision/computer-use subtasks, narrow execution with a plan handed to it, latency-critical fan-out. Not: hard reasoning, long-horizon, large-context retrieval. |
| `openai-codex/gpt-5.5` | 60.2 | agentic breadth | Terminal-native agent loops, computer-use/browser, web-research. Not: surgical in-repo refactors (Opus wins). Blended ~5-7x GLM 5.2 cost. |
...
Provider prefix note: it's `openai-codex`, not "openapi-codex". Specs are from `pi --list-models` (operational truth); context-window caps are intentional, not deficiencies (GPT-5.5/5.4-mini degrade at long context).
