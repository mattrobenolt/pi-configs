...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    label: "Memory Write",
    description: [
      "Write to one of five memory targets: long_term, project, self, user, or daily.",
      "Use this for durable prefs, repo notes, behavioral corrections, and daily logs.",
    ].join("\n"),
    parameters: Type.Object({
      ...
      mode: Type.Optional(
        StringEnum(["append", "overwrite"] as const, {
          description: "append or overwrite. Daily always appends.",
        }),
      ),
    }),
  ...
  pi.registerTool({
    ...
    label: "Scratchpad",
    description: [
      "Manage a simple checklist for follow-ups and reminders.",
      "Actions: add, done, undo, clear_done, list.",
    ].join("\n"),
    parameters: Type.Object({
      ...
      text: Type.Optional(
        Type.String({
          description: "Item text or substring match.",
        }),
      ),
    }),
    async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
  ...
  pi.registerTool({
    ...
    label: "Memory Read",
    description: [
      "Read memory files by target.",
      "Targets: long_term, project, self, user, scratchpad, daily, list.",
    ].join("\n"),
    parameters: Type.Object({
      target: StringEnum(
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      full: Type.Optional(Type.Boolean({ description: "Include the rendered context." })),
    }),
  ...
  pi.registerTool({
    ...
    label: "Memory Search",
    description:
      "Search memory files. Modes: keyword, semantic, or deep. If semantic/deep complains about missing embeddings, run `qmd embed` and retry.",
    parameters: Type.Object({
