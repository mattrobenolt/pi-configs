...
type ConversationState = {
  ...
  currentTurnMessages: PendingMessage[];
  currentTurnNotes: string[];
  currentSession: Awaited<ReturnType<typeof createAgentSession>>["session"] | null;
  typingAbortController: AbortController | null;
...
const OutputFormatParam = Type.Optional(
  Type.Union([
    Type.Literal("markdown", { description: "Return markdown output." }),
    Type.Literal("json", { description: "Return JSON-style output." }),
  ]),
);

const OutputFileParam = Type.Optional(
  Type.String({
    description: "Optional file path for the full output.",
  }),
);
...
const SlackReadParams = Type.Object({
  ...
  ts: Type.Optional(
    Type.String({
      description: "Message timestamp when reading by channel.",
    }),
  ),
  mode: Type.Optional(
    Type.Union([
      Type.Literal("message", { description: "Read one message." }),
      Type.Literal("thread", { description: "Read the full thread." }),
    ]),
  ),
...
const SlackReplyParams = Type.Object({
  url: Type.Optional(
    Type.String({
      description: "Slack message URL to reply to.",
    }),
  ),
  channel: Type.Optional(
    Type.String({
  ...
  threadTs: Type.Optional(
    Type.String({
      description: "Thread root timestamp when replying by channel.",
    }),
  ),
  text: Type.String({
  ...
  dryRun: Type.Optional(
    Type.Boolean({
      description: "Validate auth and target without posting.",
    }),
  ),
  format: OutputFormatParam,
  outputFile: OutputFileParam,
...
const SlackUserLookupParams = Type.Object({
  query: Type.String({
    description: "User ID, handle, display name, real name, or email.",
  }),
  ...
  limit: Type.Optional(
    Type.Number({
      description: `Max users. Default ${DEFAULT_USER_LOOKUP_LIMIT}; max ${MAX_USER_LOOKUP_LIMIT}.`,
      minimum: 1,
...
const SlackChannelHistoryParams = Type.Object({
  ...
  oldest: Type.Optional(
    Type.String({
      description: "Exclusive lower timestamp bound. Use nextOldest to page forward.",
    }),
  ),
  ...
  limit: Type.Optional(
    Type.Number({
      description: `Max channel messages. Default ${DEFAULT_CHANNEL_HISTORY_LIMIT}; max ${MAX_CHANNEL_HISTORY_LIMIT}.`,
      minimum: 1,
...
export async function slackSearch(
  ...
  const apiSortDir =
    input.sort_dir ?? (apiSort === "timestamp" ? ("asc" as const) : ("desc" as const));
  const startPage = Math.max(Math.trunc(input.page ?? 1), 1);
  const rawMatches = await searchMessagesRaw(
    searchQuery,
    workspaceUrl,
    rawLimit,
    options?.signal,
...
function updateConversationWidget(): void {
  ...
  const lines = [...conversations.values()].map((conv) => {
    ...
    const preview = last
      ...
      : "waiting…";
    const contextBadge =
      conv.conversationContext.length > 0 ? ` [ctx:${conv.conversationContext.length}]` : "";
    return `${icon} ${conv.label}${contextBadge}  ${preview}`;
  });
  capturedUi.setWidget("slack-conversations", lines);
}

function resolveActiveConversation(channel?: string): ConversationState | undefined {
  if (conversations.size === 0) return undefined;

  if (channel) {
    for (const [id, conv] of conversations) {
      if (id === channel || conv.label === channel || conv.channelId === channel) {
        return conv;
      }
    }
    return undefined;
  ...
  if (conversations.size === 1) {
    return [...conversations.values()][0];
  }

  return undefined;
}

async function interruptCurrentTurn(conv: ConversationState): Promise<void> {
  conv.restartCurrentTurn = true;
  conv.typingAbortController?.abort(new Error("Conversation interjected"));
  conv.typingAbortController = null;

  const session = conv.currentSession;
  if (session) {
    try {
      await session.abort();
    } catch {
...
async function generateAndSendReply(
  ...
  conv.currentTurnMessages = [...messages];
  conv.currentTurnNotes = [...turnNotes];
  conv.restartCurrentTurn = false;
  ...
  try {
    ...
    const shouldRestart =
      conv.restartCurrentTurn && !replyPosted && conv.currentTurnMessages.length > 0;
    const restartMessages = [...conv.currentTurnMessages];
    const restartNotes = [...conv.currentTurnNotes];

    conv.status = "idle";
    if (replyPosted || !shouldRestart) {
      conv.currentTurnMessages = [];
      conv.currentTurnNotes = [];
      conv.restartCurrentTurn = false;
    } else {
      conv.restartCurrentTurn = false;
    }
    ...
    if (shouldRestart) {
      await generateAndSendReply(conv, restartMessages, restartNotes);
    }
  }
}
...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      channel: Type.String({
        description: "Channel name, channel ID, or DM channel ID.",
      }),
      label: Type.Optional(
        Type.String({
          description: "Optional display name. Auto-detected if omitted.",
        }),
      ),
      systemContext: Type.Optional(
        Type.String({
          description: "Reply instructions override.",
        }),
      ),
    }),
    async execute(
      ...
      conv.pollIntervalId = setInterval(async () => {
        ...
        try {
          ...
          try {
            await generateAndSendReply(c, pending, []);
          } finally {
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      channel: Type.Optional(
        Type.String({
          description: "Channel ID or label. If omitted and only one is active, stop that one.",
        }),
      ),
    }),
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      note: Type.String({
        description: "Persistent context or instruction.",
      }),
      channel: Type.Optional(
        Type.String({
    ...
    async execute(_toolCallId, params: { note: string; channel?: string }) {
      ...
      const conv = resolveActiveConversation(params.channel);
      if (!conv) {
        const labels = [...conversations.values()].map((c) => c.label).join(", ");
        return {
      ...
      if (!note) {
        return {
          content: [{ type: "text" as const, text: "Context note is empty." }],
          details: {},
        };
      }

      conv.conversationContext.push(note);
      updateConversationWidget();
      ...
      return {
        content: [
          {
            type: "text" as const,
            text: `Saved conversation context for "${conv.label}": "${note}"`,
          },
        ],
        details: {
          note,
          label: conv.label,
          contextCount: conv.conversationContext.length,
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      note: Type.String({ description: "Immediate context or instruction." }),
      channel: Type.Optional(
        Type.String({
          description: "Channel ID or label. Defaults to the only active conversation.",
        }),
    ...
    async execute(_toolCallId, params: { note: string; channel?: string }) {
      ...
      if (!note) {
        return {
          content: [{ type: "text" as const, text: "Interjection note is empty." }],
          details: {},
        };
      }

      if (conv.status === "idle" || conv.currentTurnMessages.length === 0) {
        conv.conversationContext.push(note);
        updateConversationWidget();
        return {
          content: [
            {
              type: "text" as const,
              text: `No reply was in progress for "${conv.label}", so the note was saved as conversation context instead: "${note}"`,
            },
          ],
          details: { note, label: conv.label, mode: "context" } as Record<string, unknown>,
        };
      }

      conv.currentTurnNotes.push(note);
      await interruptCurrentTurn(conv);
      updateConversationWidget();
      ...
      return {
        content: [
          {
            type: "text" as const,
            text: `Interrupted the in-flight reply for "${conv.label}" and injected: "${note}"`,
          },
        ],
        details: { note, label: conv.label, mode: "interrupt" } as Record<string, unknown>,
      };
    },
  });
  ...
  pi.registerCommand("conversations", {
    ...
    handler: async (_args, ctx) => {
      ...
      } else if (action === "Add persistent context") {
        const note = await ctx.ui.input(
          "Conversation context",
          `Context to keep in mind for ${conv.label}`,
        );
        if (note?.trim()) {
          conv.conversationContext.push(note.trim());
          updateConversationWidget();
          ctx.ui.notify(`Saved context for "${conv.label}"`, "info");
        }
      } else if (action === "Interject current turn") {
        const note = await ctx.ui.input(
          "Interject current turn",
          `Interrupt the current unsent reply to ${conv.label}`,
        );
        if (note?.trim()) {
          if (conv.status === "idle" || conv.currentTurnMessages.length === 0) {
            conv.conversationContext.push(note.trim());
            updateConversationWidget();
            ctx.ui.notify(`No reply in flight — saved as context for "${conv.label}"`, "info");
          } else {
            conv.currentTurnNotes.push(note.trim());
            await interruptCurrentTurn(conv);
            updateConversationWidget();
            ctx.ui.notify(`Interrupted current reply for "${conv.label}"`, "info");
          }
        }
